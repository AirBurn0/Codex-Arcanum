...
local function create_cards_disable_collection(configRef, pool, rows, args)
    ...
    local create_card = function(center, i, j, _args)
        ...
        card.hover = function(self)
            ...
            self.debuff = false
            if center:is(CodexArcanum.FakeTag) then
                self.ability_UIBox_table = generate_card_ui(center)
            else
        ...
        card.click = function(self)
            ...
            self:juice_up(1, 0.5)
            self.debuff = not configRef[key]
        end
        if _args.modify_card then
            _args.modify_card(card, center)
        end
        return card
    end
...
local UIDEF = {
    ...
    tags = {
        ...
        disable = function()
            -- return create_tags_disable_collection(CodexArcanum.config.modules.Tags, CodexArcanum.pools.Tags, 1, 1)
